...
## [Unreleased] - 2026-09-07（道具牌可点用化）

### Changed

- **答题场道具牌从「信息条」改为「按钮」**：走查反馈道具看着像纯信息展示、不知道能点。道具牌弃用与题号/答对答错同款的淡墨信息胶囊（`ink-chip`），改为浅木渐变小木牌 + 「用」朱砂印 + 数量印 + 按压动效（与选项同族 `tap-deep`），可点用时带一圈缓慢呼吸微光（`item-invite`），一眼可辨可点。状态表达：双倍挂起 → 松绿描边 + 「已就绪」角标；排除已用 → 置灰 + 「已用」角标；数量为 0 且无状态时照旧隐藏。开场面板道具说明追加「答题时点击上方的道具牌即可使用」。（曾试用 `ui-slip` 木简纹理，36px 雕花端会裁掉数量印，改用 CSS 木纹渐变；截图存 `screenshots/item-chip/`。）

## [Unreleased] - 2026-09-07（答题界面主角走查优化）

### Changed

- **答题界面主角形象加大**：闯关模式与无尽模式答题场的唐小诗立绘从 `h-28`（112px）加大到最高 `h-40`（160px），无尽开场立绘同步加大到 `h-36`。尺寸随视口高度自适应（`clamp`）：390×844 满档 160px，360×640 收敛到 128/144px，320×568 矮屏回落，保证主角头顶不压 HUD、脚不没入题卡（走查脚本 `scripts/hero-style-shots.mjs`，基线与改后截图存 `screenshots/hero-style-*`）。
- **无尽模式主角位置靠上**：无尽答题场主角锚点从 `bottom-[33%]`（矮屏会被题卡挡住半身）改为钉在题卡上沿之上 `bottom-[calc(23rem+safe-area)]`，三档屏幕实测与题卡保持 12–16px 呼吸空隙。
- **无尽背景图重绘为纯风景空镜**：旧 `public/art/bg/endless.png` 里画了一个小女孩，与前景唐小诗立绘穿模（主角只露出头部）。按 `docs/art.md` 背景「必须空无一人」的既定规范用 `python3 scripts/art/backgrounds.py --only=bg-endless --force` 重生成（云海石板路 + 灯笼 + 远山楼阁，无人），并走 256 色量化压缩（2148KB → 847KB）。

## [Unreleased] - 2026-09-07（点睛联优先出题）

### Changed

- **出题取材改为「点睛联优先」**：走查发现《宿新市徐公店》虽是二年级教材诗，但补全题考的是起兴首联「篱落疏疏一径深」，体感生僻。名句多落在诗的后半（「儿童急走追黄蝶」「飞流直下三千尺」「问君能有几多愁」），因此补全题与诗名题的引句统一改为优先取靠后的句对：q1 = 最后 1 对、q2 = 倒数第 2 对、q3 = 最后 1 对反向、q4 引句 = 点睛联、q5 = 其余句对按稳定序补充。全库 10735 题重新编译，题型配比（3 补全 + 2 诗名）与取材顺序文档（`docs/content-rules.md`）同步更新；此规则对关卡、诗卡、诗库、无尽模式全局生效。

## [Unreleased] - 2026-09-07（教材篇目前置）

### Changed

- **关卡档内顺序改为按「学习常见度」铺位（ADR-0020 修订）**：走查发现第 1 关仍会抽到千家诗冷门五绝等生僻篇目——篇幅相近不等于常见。新增每诗 `studyRank` 学习常见度（bank 契约 v4）：0 教材篇目、1000 通识读本（三字经/声律启蒙/蒙学唐诗/千家诗/唐诗三百首）、2000 经典、3000 雅致（花间集）。档内出题顺序改为常见度分组依次铺开，**教材篇目严格前置**：第 1 关全部是《春晓》《山行》《赠汪伦》级课本诗，第 31–40 关全程初中教材篇目，生僻诗只出现在每档末尾。邻档补足同样常见度优先。
- 档内洗牌从「每 2 关一段」改为「常见度组内整组洗牌」：组间顺序全服一致，组内顺序与每关诗单因人而异（防透题保留在诗单粒度）；同一玩家题目仍永远固定。篇幅不再决定档内顺序（学过与否比长短更接近真实难度，有意取舍）。

### Added

- 新增测试：教材篇目严格前置（第 1 关全教材 / 第 10 关全生僻）、常见度压过篇幅、组间顺序一致与组内因人而异；`validate_content.py` 校验 `studyRank` 分组口径（0 当且仅当教科书选诗）。

### Docs

- `docs/adr/0020-level-difficulty-tiers.md` 决策 4/后果修订；`docs/content-rules.md` 第四节扩为「学段难度与学习常见度」（含 studyRank 分组表）；`docs/game-design.md`、`CONTEXT.md`、`README.md` 同步口径。
