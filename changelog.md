...
## [Unreleased] - 2026-09-07（学段难度分档）

### Added

- **关卡学段难度循序渐进（ADR-0020）**：50 关分为 5 个学段难度档（小学·低 / 小学·中 / 小学·高 / 初中 / 高中），每 10 关一档；每关只出本档的诗，档内按篇幅先短后长、逐关递进。第 1 关开局即「静夜思、竹里馆」式短经典。
- 全部 2147 首诗编译出学段难度 `difficulty` 1–5（bank 契约 v3）：教科书选诗按章节年级直接推导，其他文集按「蒙学属性 + 篇幅」代理定档；规则唯一权威见 `docs/content-rules.md` 第四节。分档分布 T1 232 / T2 72 / T3 471 / T4 727 / T5 645，T2 不足 130 首时由 T3 按篇幅就近补足（`buildTierBands`，一首诗只进一个档，50 关 650 个诗位零重复）。
- 新增纯函数 `buildTierBands` / `bandOrder` / `tierForLevel` / `tierLabel` 与 `TIER_LABELS`（`src/lib/game/levels.ts`），配套 8 个新测试：档位映射、档位纯净、全 50 关零重复、档内递进、邻档补足、难度带一致性与玩家隔离。
- 关卡 UI 标注学段：列表卡状态行显示学段标签（替代「可挑战」文案，未解锁卡显示「学段 · 未解锁」）；答题开场面板一行「本关诗文 · 学段」；题卡出处行与作答报告出处行尾随学段标签。

### Changed

- 出题算法从「全库洗牌切段」改为「学段分档 + 档内分段洗牌」：难度带全服一致，段内顺序因人而异（防透题承诺不变）；同一玩家题目仍永远固定。`PlanPoem`/`Poem` 契约新增 `difficulty`（客户端加载缺省回落中档）。
- 关卡列表卡状态行文案「可挑战」由学段标签替代，下一关保留朱砂圆点提示。

### Docs

- 新增 `docs/adr/0020-level-difficulty-tiers.md` 并更新 ADR 索引（0018 的「无难度维度」条款标注被修订）；`README.md`、`CONTEXT.md`、`docs/game-design.md`（支柱/当前不做/算法表/页面职责/验收项）、`docs/content.md`、`docs/content-rules.md`（新增第四节「学段难度」与质检第 6 条）、根 `AGENTS.md`（产品事实与红线）同步。
...
- **主页用户数据面板（远征牌）**：位置下移（15.5% → 19.5%）并换用更透的 `paper-plate-airy` 纸底（74%/62% + 强磨砂），844/667/640 三档屏高验证副标题「探索中国古诗词之美」不再被遮挡；庭院场景从面板后隐约透出。
...
- 翻页玉钮去掉外层衬底容器：只裸露玉盘箭头贴图，字衬圆钮仅在贴图缺失时回退出现。
