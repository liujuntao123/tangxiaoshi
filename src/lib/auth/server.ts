...
const extraTrustedOrigins: string[] = (env("BETTER_AUTH_TRUSTED_ORIGINS") ?? "")
  ...
  .filter(Boolean);
const APP_PUBLIC_HOSTS: string[] = ["tangxiaoshi.aizhi.site", "*.aizhi.site"];
const baseURL = explicitBaseURL ?? {
  // Include loopback hosts so dynamic baseURL resolves for local email/password
  // (not only the preview wildcard).
  allowedHosts: [...previewAllowedHosts, "localhost", "127.0.0.1", "[::1]", ...APP_PUBLIC_HOSTS],
...
const trustedOrigins: string[] = explicitBaseURL
  ? [explicitBaseURL, ...LOCAL_DEV_ORIGINS, ...APP_PUBLIC_ORIGINS, ...extraTrustedOrigins]
  : [
