...
项目是竖屏 Web 古诗文游戏《唐小诗环游记》。当前主线版本是“墨潮远征”（关卡制）：首页是远征台，`/levels` 是关卡列表，`/levels/$levelId` 是关卡答题场（每关 10 题、答对 6 题过关、星星与道具奖励）；`/play/$poemId` 是资料库诗卡答题（5 题 + 诗印）。当前玩法事实以 `CONTEXT.md`、`docs/game-design.md`、`docs/adr/0018-level-based-expedition.md` 和代码为准。
...
- 关卡系统保持直白：平铺关号（无难度维度）、道具名称即用途；不把「墨路」「诗签」「修页奖励」这类旧隐喻加回来，也不把道具扩展成复杂装备树。
