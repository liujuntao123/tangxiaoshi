...
项目是竖屏 Web 古诗文游戏《唐小诗环游记》。当前主线版本是“墨潮远征”：首页是远征台，`/tour` 是三节点墨路选择，`/play/$poemId` 是诗页修复答题场。当前玩法事实以 `CONTEXT.md`、`docs/deep-reboot-brief.md`、`docs/adr/0016-ink-tide-expedition.md` 和代码为准。

旧的“朝代 → 诗人 → 关卡 / 地图 / 妖怪 / 钥匙”只允许出现在历史 ADR 的背景说明中，不得作为新功能设计依据。

## 2. 文档同步铁律

修改下列任一项目事实时，必须在同一变更中同步文档：

- 主循环、路线、诗火、诗签、修页奖励、结局或入口命名：更新 `README.md`、`CONTEXT.md`、对应 `docs/*.md`，必要时新增/更新 ADR。
- 存档字段、迁移、远征持久化边界：更新 `docs/content.md`、ADR、README 技术说明和 `changelog.md`。
...
- `PlayerSave` 是按用户 ID 保存的永久成绩：诗卡通关、诗印、最佳分、总分、成就、作者遇见记录和墨潮试炼纪录。
- 当前远征进行时状态不是 `PlayerSave` 字段，而是浏览器 `localStorage` 的 `tangxiaoshi.expedition.v1`。它可跨刷新恢复，但不跨设备、不跨浏览器，也不做账号隔离；不得在文档中称为账号级云存档。
- 客户端状态必须有 normalize、`try/catch` 和无存储能力时的降级路径。若未来改为账号级同步，必须新增 migration、更新 `PlayerSave`/validator/读写和 ADR。
- 新的数据库结构化字段必须使用有序 SQL migration；Neon 与 PGLite 必须兼容。

## 4. 当前玩法红线

- 保持古风 Q 版绘本、短局答题、主动“收句”、可读错题反馈。
- 不恢复妖怪、血条、攻击/受击帧、钥匙门控、旧地图解锁、排行榜压迫或付费/抽卡系统。
- 远征只使用轻量状态：诗火、连击、当前诗签、一个修页奖励；不扩展成复杂装备树。
