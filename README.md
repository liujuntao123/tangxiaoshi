...
| 入口 | 规则 |
| --- | --- |
| 墨潮远征（主线） | 首页「继续闯关」直达下一关，或进 `/levels` 选关。共 50 关平铺、顺序解锁；每关 10 道题，答对 6 题过关。 |
...
| 墨潮试炼 | 无尽模式：全部题池随机，一题答错本局结束，记录最高连对和最高分。 |
...
- 永久成绩按用户 ID 存入 `player_saves`（`migrations/0001–0005`）：诗卡通关、诗印、最佳分、环游总分、成就、已遇作者、墨潮试炼纪录、关卡星星（`level_stars`）与道具库存（`items`）。
