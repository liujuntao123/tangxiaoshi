...
| 专属题序 | 每关题目由你的账号专属生成：同一关卡你的题目永远固定，不同玩家互不相同，无法互相透题；整个战役 650 个诗位零重复。每关只出本学段档的诗，且教材里学过的篇目优先，生僻诗只会出现在每档末尾。 |
...
- 关卡题目不存档：由 userId 即时确定性生成（学段分档 → 常见度分组 → 组内 FNV-1a 种子洗牌），同一账号永远同一份题。
