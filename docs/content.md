...
历史说明：`validate_story.py` 已删除（其校验对象 `story.json` 并入 bank.json，质检统一走
`validate_content.py`）；旧 `scripts/content/story/` 目录（world/canonical/chapters）已删除。
...
endlessBestScore / metAuthors`，外加诗卡成绩 `poemRecords / totalScore`（`migrations/0004_poem_records.sql`，
读取端统一 `normalizeSave` 规范化）。层级重构时旧进度全部重置、无尽最高分保留（`migrations/0003_save_v2.sql`）。

## 当前状态

- **11 个文集全部已编译上线**：89 章 / 281 位作者 / 2147 张诗卡 / 10735 题；成就 302 枚（作者 281 + 文集 11 + 朝代 10）。
- 新增内容 = `catalog.py` 加条目 + 编译 + 素材生成。
